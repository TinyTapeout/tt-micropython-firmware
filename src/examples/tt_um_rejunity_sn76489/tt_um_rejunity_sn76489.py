...
Adapted from https://github.com/rejunity/tt05-psg-sn76489/blob/main/test/test.py
...
async def test_output_amplitudes(dut):
    ...
    for chan in '123':
        ...
        for step, vol in enumerate(amplitudes):
            assert vol > prev_volume or \
              (prev_volume == vol and step < 4), f"{vol} > {prev_volume} or ({prev_volume} == {vol} and {step} < 4)"
...
async def assert_output(dut, frequency=-1, period=-1, constant=False, noise=False, v0 = ZERO_VOLUME, v1 = MAX_CHANNEL_VOLUME):
    ...
    for i in range(cycles_to_collect_data//clocks_to_step):
        ...
        out = get_output(dut)
        # dut._log.error(f' out: {out}')
        new_state = out > mid_volume
    ...
    if not constant:
        ...
        assert frequency * (1.0-max_error) <= measured_frequency \
            and measured_frequency <= frequency * (1.0+max_error), f"measured freq {measured_frequency:3.2f} expected {frequency:3.2f}"

    pulses_to_collect2 = pulses_to_collect*2
    assert pulses_to_collect2 * (1.0-max_error) <= state_changes \
        and state_changes <= pulses_to_collect2 * (1.0+max_error), f"assert_ouput fail: pulses {pulses_to_collect2}, states {state_changes}"
    
def global_overrides(selection, master_clock, internal_clock_div):
    global SEL
    global MASTER_CLOCK
    global CHIP_INTERNAL_CLOCK_DIV
    ...
    if internal_clock_div is not None:
        CHIP_INTERNAL_CLOCK_DIV = internal_clock_div
    ...
    if SEL == 0 or SEL == "":
        try:
            ...
            if MASTER_CLOCK == "NTSC":
                MASTER_CLOCK = 3_579_545
            pass
        try:
            CHIP_INTERNAL_CLOCK_DIV = int(CHIP_INTERNAL_CLOCK_DIV)
        except:
            pass
